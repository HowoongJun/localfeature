...
class CEventPointNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        kpt = self.pool(x)
        kpt = self.relu(self.conv2_1_1(kpt))
        kpt = self.relu(self.conv2_1_2(kpt))
        kpt = self.pool(kpt)
        kpt = self.relu(self.conv3_1_1(kpt))
        kpt = self.relu(self.conv3_1_2(kpt))
        kpt = self.pool(kpt)
        kpt = self.relu(self.conv4_1_1(kpt))
        kpt = self.relu(self.conv4_1_2(kpt))
